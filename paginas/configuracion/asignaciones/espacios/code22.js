...
$( document ).ready(function() {
    ...
    $(document).on('change','.booleano_activo_espacios',function(event){   
        if (paginaActiva.code == 22){
            ...
            updateFieldBoolean("Espacios",
                "mediciones.espacios",
                {field:"idespacio",value:id,type:'integer'},
                {name:'activo',value:activo,type:'boolean'},
                {name:'idespacio',value:id},
                reporteEspacios,
                "#checkActivo"
            );
        }
    ...
    $(document).on('click','#btnNuevo',function(event){
        if (paginaActiva.code == 22){
            ...
            $("#SelectEdificio").val(indexEdificio);
        ...
        if (paginaActiva.code == 22){
            ...
            if ($("#hdOperacion").val()=="Ingresar") {
                if (validar("#FormaParametros")) {
                        guardar_espacios();
                        confirmarModificar(tablaEspacios,{
                            titulo:'Confirmación',
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                    if (data[0].tipo == "success"){
                        ...
                        guardar_log(data[0].return[0]["idespacio"],elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                if (data[0].tipo == "success"){
                    ...
                    guardar_log(idEspacio,elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
                }
                else {
                    console.log(data);
                    error2({
