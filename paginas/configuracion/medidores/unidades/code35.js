...
$( document ).ready(function() {
    ...
    $(document).on('change','.booleano_activo_unidades',function(event){   
        if (paginaActiva.code == 35){
            ...
            updateFieldBoolean("Unidades",
                "mediciones.unidades",
                {field:"idunidad",value:id,type:'integer'},
                {name:'activo',value:activo,type:'boolean'},
                {name:'idunidad',value:id},
                reporteUnidades,
                "#checkActivo"
            );
        }
        ...
        if (paginaActiva.code == 35){
            ...
            if ($("#hdOperacion").val()=="Ingresar") {
                if (validar("#FormaParametros")) {
                        guardar_unidades();
                        confirmarModificar(tablaUnidades,{
                            titulo:'Confirmación',
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                    if (data[0].tipo == "success"){
                        ...
                        guardar_log(data[0].return[0]["idunidad"],elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                if (data[0].tipo == "success"){
                    ...
                    guardar_log(idunidad,elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
                }
                else {
                    console.log(data);
                    error2({
