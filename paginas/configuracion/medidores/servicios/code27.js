...
$( document ).ready(function() {
    ...
    $(document).on('change','.booleano_activo_servicios',function(event){   
        if (paginaActiva.code == 27){
            ...
            updateFieldBoolean("Servicios",
                "mediciones.servicios",
                {field:"idservicio",value:id,type:'integer'},
                {name:'activo',value:activo,type:'boolean'},
                {name:'idservicio',value:id},
                reporteServicios,
                "#checkActivo"
            );
        }
        ...
        if (paginaActiva.code == 27){
            ...
            if ($("#hdOperacion").val()=="Ingresar") {
                if (validar("#FormaParametros")) {
                        guardar_servicios();
                        confirmarModificar(tablaServicios,{
                            titulo:'Confirmación',
    ...
    function cargarservicio(record){
        ...
        $("#TextPrefijo").val(record.prefijo);
        $("#TextDescripcion").val(record.descripcion);
        $("#TextMedida").val(record.valor);
        $("#TextUnidad").val(record.unidad);
    ...
    function guardar_servicios(){
        ...
        elementos["campos"].push({
            ...
            "prefijo"      : $("#TextPrefijo").val(),
            "descripcion" : $("#TextDescripcion").val(),
            "valor" : $("#TextMedida").val(),
            "unidad" : $("#TextUnidad").val(),
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                    if (data[0].tipo == "success"){
                        ...
                        guardar_log(data[0].return[0]["idservicio"],elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
    ...
    function actualizar_servicios(idservicio){
        ...
        elementos["types"] = "text,text,text,text,text,boolean,integer";
        elementos["campos"].push({
            "update":{
                "nombre"      : $("#TextNombre").val(),
                "prefijo"     : $("#TextPrefijo").val(),
        ...
        $.ajax({
            ...
            success: function( data, textStatus, jQxhr ){
                if (data[0].tipo == "success"){
                    ...
                    guardar_log(idservicio,elementos["tableName"],elementos["operacion"],JSON.stringify(elementos["campos"]),usuarioActivo);
                }
                else {
                    console.log(data);
                    error2({
    ...
    function updaterecord(idservicio){
        ...
        record.prefijo = $("#TextPrefijo").val();
        record.descripcion = $("#TextDescripcion").val();
        record.valor = $("#TextMedida").val();
        record.unidad = $("#TextUnidad").val();
        ...
        record2.valor = record.valor;
        record2.unidad = record.unidad;
