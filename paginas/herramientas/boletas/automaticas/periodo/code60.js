...
$( document ).ready(function() {
    ...
    $(document).on('click','.descargar_boletas_periodo_automaticas',function(event){  
        if (paginaActiva.code == 60){
           ...
           // arrayId = [];
           // arrayId.push(id);
           var tipo = ($("#SelectEdificio").val()!=15)?'detallada':'simple';
            generar_boleta_array([$(this).attr("data-idremarcacionasignacion")],this,tipo,$("#SelectEdificio").val());
        }
    });
    ...
    $(document).on('click','#descargarTodasPeriodo',function(event){
        if (paginaActiva.code == 60){
           ...
           // });
           var tipo = ($("#SelectEdificio").val()!=15)?'detallada':'simple';
            generar_boleta_array(Tabla.rows( { filter : 'applied'} ).data().map(boleta => boleta.idremarcacionasignacion),this,tipo,$("#SelectEdificio").val());
        }
    });
...
*/
            ajaxParams = {
                ...
                data:function(d){
                    ...
                    d.queries=[{
                        fieldsSelect:["idremarcacionasignacion","desde","hasta","fecha::date","extract('year' from fecha) as ano","lpad(extract('month' from fecha)::text,2,'0') as mes","vremarcacionesasignaciones.cons_nombre","vremarcacionesasignaciones.codigo","sumavaloresfacturados.total","sumavaloresfacturados.total*1.19 as totaliva"],
