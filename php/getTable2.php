<?php
    ...
    function dibujar_posiciones($groupName,$groupValue,$table,$id,$posicion,$actual,$total){
        ...
        if ($total == 1){
            ...
            if ($posicion == 1) {
                $str .= sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='+' name='posicion' class='btn btn-info btn-xs posicion_%s'><i class='fa fa-arrow-down'></i></button>",$id,$groupStr,$posicion,$total,$table).
                        sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='-' name='posicion' class='btn btn-default btn-xs disabled'><i class='fa fa-arrow-up'></i></button>",$id,$groupStr,$posicion,$total);
            }
            elseif ($posicion == $total){
                $str .= sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='+' name='posicion' class='btn btn-default btn-xs disabled'><i class='fa fa-arrow-down'></i></button>",$id,$groupStr,$posicion,$total).
                        sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='-' name='posicion' class='btn btn-info btn-xs posicion_%s'><i class='fa fa-arrow-up'></i></button>",$id,$groupStr,$posicion,$total,$table);
            }
            else {
                $str .= sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='+' name='posicion' class='btn btn-info btn-xs posicion_%s'><i class='fa fa-arrow-down'></i></button>",$id,$groupStr,$posicion,$total,$table).
                        sprintf("<button id='%s' %s data-posicion='%s' data-total='%s' data-operacion='-' name='posicion' class='btn btn-info btn-xs posicion_%s'><i class='fa fa-arrow-up'></i></button>",$id,$groupStr,$posicion,$total,$table);
            ...
            foreach($_POST["show"]["fields"] as $showField){
                ...
                if ($type != "bool") {
                    if ($showField != 'posicion') {
                        ...
                        $record["posicion"] = dibujar_posiciones($groupName,$groupValue,$realtablename,$registro[$_POST['primaryField']],$registro[$showField],$actual,$registro[$totalName]);
                    }
                }
                else{
                    ...
                    $record[$showField] = sprintf('<label class="cl-switch cl-switch-green"><input id="%s" class="booleano_%s_%s" type="checkbox" %s %s><span class="switcher"></span></label>',$id,$showField,$realtablename,$activo,$hideStr);
